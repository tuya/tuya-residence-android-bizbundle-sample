...
import com.tuya.smart.sr.srdemo.RowSpaceLine
        ...
        Button(onClick = {
            Global.currentSite?.homeId?.let {
                if (TextUtils.isEmpty(name) || TextUtils.isEmpty(email) || TextUtils.isEmpty(password)) {
                    Toast.makeText(context, "Name or email or password cannot be empty", Toast.LENGTH_SHORT).show()
                    return@Button
                }
                ...
                TuyaSmartResidenceSdk.access().addPasswordAccess(it.toString(),
                    ...
                    object : Business.ResultListener<PasswordResult> {
                        override fun onFailure(p0: BusinessResponse?, p1: PasswordResult?, p2: String?) {
                            Toast.makeText(context, "addPasswordAccess failure:${p0?.errorMsg}", Toast.LENGTH_SHORT)
                                .show()
                        }

                        override fun onSuccess(p0: BusinessResponse?, p1: PasswordResult?, p2: String?) {
                            Toast.makeText(context, "addPasswordAccess success", Toast.LENGTH_SHORT)
                                .show()
